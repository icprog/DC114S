...
int key_list_keyset(key_list_t *list,key_t_tlv* array,int array_size)
{    
...
static key_list_node_t* key_list_get_node(key_list_t *list,key_t_tlv key) 
{
    key_list_node_t *current = list->header;    
    while (current != NULL) {
...
int key_list_find_key(key_list_t *list,key_t_tlv key) 
{
    return key_list_get_node(list,key) != NULL;
}

int key_list_add(key_list_t *list,key_t_tlv key,value_t value)
{
...
int key_list_get(key_list_t *list,key_t_tlv key,value_t *value) 
{
...
int key_list_edit(key_list_t *list,key_t_tlv key,value_t value)
{
    key_list_node_t* node = key_list_get_node(list,key);
    if (node == NULL) {
        return -1;
...
int key_list_delete(key_list_t *list,key_t_tlv key) 
{
    key_list_node_t* node = key_list_get_node(list,key);
    if (node == NULL) {
        return -1;
